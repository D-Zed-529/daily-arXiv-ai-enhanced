...
TL;DR: SCAR提出了一种鲁棒的ECG-语言预训练框架，通过对抗性移除进行语义补偿，提高心电图在部分观测下的语义对齐鲁棒性


<details>
  <summary>Details</summary>
Motivation: 现有ECG-语言预训练方法在完全观测ECG设置下研究，但实际中由于电极脱落、运动伪影或信号损坏，可能导致诊断关键导联或时间段的缺失，这会严重降低跨模态语义对齐性能

Method: 提出SCAR框架：1) 使用可微分对抗掩码器移除训练期间最关键的时空ECG标记，迫使ECG编码器学习在主要诊断证据缺失时仍与临床文本保持语义对齐的表示；2) 配备语义监督的自适应选择器，学习重新加权剩余可见标记，并利用次要但具有诊断信息的形态学线索进行补偿；3) 引入反事实缺失解析评分(CMRS)来量化特征在缺失情况下保留诊断语义的能力

Result: 在6个数据集上的实验表明，SCAR在联合导联和时间缺失情况下持续提高语义鲁棒性，特别是在主要诊断证据不可用的困难案例中表现出明显优势，同时产生更强的线性探测可迁移性

Conclusion: SCAR通过显式建模对部分观测的鲁棒性，解决了ECG-语言预训练中的缺失数据问题，提高了在现实临床场景中的实用性和可靠性
...
TL;DR: IBAM提出了一种基于神经辅助物理框架的可解释电池老化建模方法，通过2D老化指纹提供比单一SOH更丰富的退化行为解释。


<details>
  <summary>Details</summary>
Motivation: 传统电池健康状态(SOH)作为单一标量指标解释性有限，相似SOH的电池可能表现出完全不同的退化行为，缺乏可解释性阻碍了电池的优化运行。
...
Result: 在短、中、长寿命电池上，IBAM在不同老化阶段始终提供最佳的物理模型保真度，清晰解释了不同寿命电池的退化机制和指纹模式，指纹支持可解释的电池健康评估并可为电池控制选择提供信息。

Conclusion: IBAM仅使用电池管理系统的常规日志数据，无需额外诊断测试即可输出2D老化指纹，通过捕获电池曲线范围的极化电压损失和放电末端的尾部损失，提供了卓越的可解释性，支持更优的电池管理决策。
...
TL;DR: 重新审视语义MIMO系统中的线性预编码（MF和ZF），发现与传统MIMO相比，语义MIMO对干扰和CSI误差的敏感性降低，在高SINR区域性能稍差，但MF能达到与ZF相当的语义性能，简化了系统实现复杂度。


<details>
  <summary>Details</summary>
Motivation: 研究在生成式AI赋能的语义MIMO系统中，传统MIMO系统的干扰、信道状态信息准确性和可扩展性限制是否仍然关键。探索语义通信是否能够放宽对激进干扰抑制和高精度CSI的需求。

Method: 基于生成推理模型和Lipschitz连续性假设进行理论分析，比较匹配滤波（MF）和迫零（ZF）预编码在语义MIMO系统中的性能。通过仿真验证理论分析，评估完美和不完美CSI条件下的语义性能。

Result: 理论分析显示语义MIMO对干扰和信道不完美的敏感性降低，在高SINR区域性能略低于传统MIMO系统。仿真结果表明，在完美和不完美CSI条件下，MF都能达到与ZF相当的语义性能。

Conclusion: 语义MIMO放宽了对激进干扰抑制和高精度CSI的需求，同时通过降低计算和实现复杂度提高了系统可扩展性。MF预编码在语义MIMO系统中具有实用优势。
...
TL;DR: 结合信号门控与本地振荡器频率切换，可在无需昂贵接收器并行化的情况下实现离线相干接收的带宽扩展，实验验证了最高288 GBaud符号率的性能


<details>
  <summary>Details</summary>
Motivation: 传统相干接收系统在扩展带宽时需要昂贵的接收器并行化，这增加了系统成本和复杂性。研究旨在开发一种无需并行化就能扩展带宽的经济高效方法

Method: 提出信号门控与本地振荡器频率切换相结合的技术。通过门控信号并切换本地振荡器频率，实现宽带信号的分解接收，避免传统并行化方案

Result: 实验验证了该概念在高达288 GBaud符号率下的可行性，成功实现了带宽扩展而无需接收器并行化

Conclusion: 信号门控与本地振荡器频率切换相结合提供了一种经济高效的带宽扩展方案，为高速相干通信系统提供了有前景的替代方案
...
### [5] [Real-Time and Scalable Zak-OTFS Receiver Processing on GPUs](https://arxiv.org/abs/2604.02266)
...
TL;DR: 提出基于GPU的实时Zak-OTFS接收机架构，通过硬件-算法协同设计利用延迟-多普勒域信道稀疏性，显著降低计算复杂度，实现高吞吐量实时处理


<details>
  <summary>Details</summary>
Motivation: OTFS调制相比传统OFDM在高移动性信道中具有优越鲁棒性，但其显式延迟-多普勒域表示在DD网格尺寸增大时带来巨大信号处理复杂度，需要解决实时处理挑战

Method: 采用硬件-算法协同设计，利用DD域信道稀疏性，设计紧凑矩阵运算关键处理阶段、无分支迭代均衡器，以及结构化稀疏信道矩阵表示，显著减少计算和内存开销

Result: 系统在DD网格尺寸(16384,32)、16QAM调制、245.76MHz带宽下实现高达906.52Mbps吞吐量，满足99.9百分位实时处理截止时间，在多种CPU和GPU平台上展示良好可扩展性和鲁棒性能

Conclusion: 提出的计算感知Zak-OTFS接收机设计通过硬件-算法协同优化，有效解决了OTFS实时处理复杂度问题，为下一代高移动性通信系统提供了可行的实时处理解决方案
...
### [6] [Channel Measurements and Modeling based on Composite Environmental Factor for Urban Street-Canyon Intersections](https://arxiv.org/abs/2604.01767)
*Xinwen Chen,Ruisi He,Mi Yang,Zhengyu Zhang,Junzhe Song,Jiahui Han,Haoxiang Zhang*

Main category: eess.SP

TL;DR: 本文提出了一种基于5.8GHz信道测量的环境相关统计信道模型，用于表征城市街道峡谷交叉口的无线信道特性，通过构建复合环境因子来捕捉不同建筑条件的影响。


<details>
  <summary>Details</summary>
Motivation: 在城市环境中，车联网通信需要准确的信道表征。街道峡谷交叉口由于建筑遮挡和丰富的多径传播，会严重降低链路可靠性。现有信道模型未能充分捕捉建筑分布对信道特性的影响，特别是在交叉口这种特殊环境布局下。

Method: 基于5.8GHz信道测量数据，构建复合环境因子来表征交叉口的环境差异。将该因子融入3GPP路径损耗模型，并进一步关联到小尺度信道参数。使用二阶信道统计量验证模型准确性。

Result: 结果表明，所提出的模型能够有效表征不同建筑条件下城市街道峡谷交叉口信道的传播特性。模型为城市车辆场景下的信道仿真和性能评估提供了物理可解释且统计有效的框架。

Conclusion: 该研究提出的环境相关统计信道模型成功解决了现有模型在捕捉建筑分布影响方面的不足，为V2X通信系统在城市街道峡谷交叉口的可靠设计提供了重要工具。
...
### [7] [1-bit Quantized Continuous Aperture Arrays](https://arxiv.org/abs/2604.01780)
*Kuranage Roche Rayan Ranasinghe,Getuar Rexhepi,Zhaolin Wang,Giuseppe Thadeu Freitas de Abreu*

Main category: eess.SP

TL;DR: CAPA接收架构与1-bit ADC结合，在瑞利衰落下存在分集阶数损失，但在LoS条件下可完全消除1-bit量化惩罚，达到未量化AWGN性能界。


<details>
  <summary>Details</summary>
Motivation: 研究连续孔径阵列(CAPA)接收架构与低成本1-bit ADC的交互作用，探索CAPA能否缓解传统离散天线阵列中1-bit ADC带来的严重非线性失真问题。

Method: 采用矩匹配近似(MMA)方法，基于Gamma矩匹配的空间特征值分布，推导瑞利衰落下的符号错误概率(SEP)闭式近似；对于LoS传播，分析完美空间和相位对齐条件下的性能。

Result: 瑞利衰落下，CAPA存在由Jensen不等式控制的分集阶数惩罚；LoS条件下，CAPA在完美对齐时可完全消除1-bit量化惩罚，达到未量化AWGN性能界，无需像离散系统那样加倍天线数量。

Conclusion: CAPA架构与1-bit ADC结合在LoS条件下具有显著优势，可完全消除量化失真，为6G系统提供了一种经济高效的物理层解决方案。
...
### [8] [MIMO Capacity Enhancement by Grating Walls: A Physics-Based Proof of Principle](https://arxiv.org/abs/2604.01786)
*Xiaolu Yang,Oscar Cespedes Vicente,Christophe Caloz*

Main category: eess.SP

TL;DR: 通过边界工程（特别是光栅涂层墙壁）被动增强MIMO室内信道容量，利用墙壁反射率和衍射诱导的角度重分布来丰富信道特征结构


<details>
  <summary>Details</summary>
Motivation: 研究如何通过被动方式（无需主动信号处理）增强MIMO系统的频谱效率，特别是在室内环境中通过工程化边界特性来优化传播信道

Method: 建立二维室内原理验证模型，使用电磁格林函数构建传播信道，比较四种边界类型：自由空间、干墙、完美电导体（PEC）和二元光栅涂层墙壁

Result: 光栅涂层墙壁能丰富非视距多径场、降低信道相关性，并在广泛的接收器位置范围内增强空间复用能力；容量提升源于墙壁有效反射率（能量限制和再辐射）和衍射诱导角度重分布的共同作用

Conclusion: 边界工程特别是光栅涂层墙壁能有效被动增强MIMO频谱效率，这种增强不仅源于衍射效应，更是墙壁反射率和角度重分布协同作用的结果，为室内无线系统设计提供了新思路
...
### [9] [Empirical and Statistical Characterisation of 28 GHz mmWave Propagation in Office Environments](https://arxiv.org/abs/2604.01814)
*Ayodeji Bolanle Balogun,Sokipriala Jonah*

Main category: eess.SP

TL;DR: 该研究通过TMYTEK测量系统验证了28GHz毫米波在室内的传播特性，发现路径损耗符合自由空间理论，材料衰减特性与厚度假设不符，反射器优化可显著提升覆盖性能。
...
Method: 使用TMYTEK测量系统进行室内毫米波传播测量，采用统计检验和置信区间分析验证路径损耗模型，分析不同材料（办公隔板和显示板）的穿透损耗，并研究反射器优化对覆盖增强的效果。
...
Conclusion: 研究证实经典传播模型适用于室内毫米波部署，材料衰减特性需要重新评估，反射器优化是有效的覆盖增强手段。结果为室内毫米波系统设计提供了新的实证基准和实用指导。
...
### [10] [Evaluation of gNB Monostatic Sensing for UAV Use Case](https://arxiv.org/abs/2604.02205)
...
<details>
  <summary>Details</summary>
Motivation: 3GPP Release 19已启动ISAC标准化工作，包括单站感知信道模型、评估场景和性能评估方法。虽然这些共同假设为ISAC评估提供了重要基础，但可重复的端到端研究仍需要透明的感知实现。本研究旨在填补这一空白，为无人机场景提供可重复的5G NR基站单站感知评估。

Method: 采用5G NR下行CP-OFDM波形和定位参考信号(PRS)，遵循3GPP UMa-AV场景假设。提出了一个端到端处理链，包括多目标检测和3D定位算法。开发并发布了5GNRad仿真器，支持可重复的基线研究和进一步研究。

Result: 在考虑的场景中实现了超过70%的检测概率和低于5%的虚警率。对于正确检测的目标，定位误差在几米范围内：垂直方向90%分位误差为4米，水平方向为6米。发布了5GNRad仿真器以支持可重复研究。

Conclusion: 研究表明5G NR基站单站感知在无人机场景中具有可行性，能够实现有效的多目标检测和3D定位。发布的5GNRad仿真器为ISAC研究提供了可重复的基线工具，有助于推动3GPP ISAC标准化进程和进一步研究。
...
### [11] [ReVAR: A Data-Driven Algorithm for Generating Aero-Optic Phase Screens](https://arxiv.org/abs/2604.02326)
...
<details>
  <summary>Details</summary>
Motivation: 气动光学效应研究中需要大量真实数据，但现有获取方法（实验、CFD、简单相位屏算法）存在成本高、计算量大、数据量有限或统计特性不准确等问题，数据驱动方法虽有改进但仍存在质量有限、统计不准确、算法复杂等缺陷

Method: 提出ReVAR算法：1）长程自回归模型，结合标准自回归和数据的低通滤波器组，同时拟合短程和长程时间统计特性；2）空间再白化步骤，将实测数据转换为时空不相关的白噪声；3）通过反向过程使用白噪声输入生成合成数据

Result: 在两个实测湍流边界层数据集上验证，ReVAR在时间功率谱和其他关键指标上比两种传统相位屏生成方法和现有单时滞自回归模型更好地匹配实测数据

Conclusion: ReVAR算法能够高效生成统计特性匹配实测数据的合成气动光学数据，解决了现有方法的局限性，为气动光学效应研究提供了高质量数据源
...
TL;DR: DySCo是一个动态语义压缩框架，通过熵引导动态采样、分层频率增强分解和跨尺度交互混合器，有效解决长时间序列预测中的噪声和冗余问题，提升主流模型的长时依赖捕捉能力。


<details>
  <summary>Details</summary>
Motivation: 长时间序列预测中，扩展回看窗口理论上能提供更丰富的历史上下文，但实践中常引入无关噪声和计算冗余，阻碍模型有效捕捉复杂的长时依赖关系。

Method: 提出动态语义压缩框架DySCo：1）熵引导动态采样机制自主识别并保留高熵片段，压缩冗余趋势；2）分层频率增强分解策略分离高频异常和低频模式；3）跨尺度交互混合器动态融合全局上下文与局部表示。

Result: 实验表明DySCo可作为通用即插即用模块，显著增强主流模型捕捉长时相关性的能力，同时降低计算成本。

Conclusion: DySCo框架通过动态语义压缩有效解决了长时间序列预测中的噪声和冗余问题，提升了模型的长时依赖捕捉效率，具有通用性和实用性。
...
Method: 引入端到端框架，使用实际中断结果作为监督训练LLM生成校准的概率预测
...
TL;DR: 提出在线机器学习加速的多分辨率优化框架，用于工业综合能源系统设计验证，通过ML引导控制器减少架构与运行性能差距并降低高保真模型评估需求。


<details>
  <summary>Details</summary>
Motivation: 工业过程综合能源系统设计需要跨多保真度的优化和验证模型，但模型间的不匹配导致性能损失来源不明，架构到运行性能差距难以量化，需要高效方法进行设计验证。

Method: 提出在线机器学习加速的多分辨率优化框架：1) 多目标架构优化选择系统配置和组件容量；2) ML加速的多分辨率滚动时域最优控制策略，基于预测不确定性自适应调度优化分辨率，利用精英低保真解热启动高保真求解。

Result: 在1MW工业热负荷试点案例中，多分辨率策略相比基于规则的控制器将架构到运行性能差距减少42%，相比无ML引导的多保真方法减少34%高保真模型评估需求，实现更快更可靠的设计验证。

Conclusion: 该框架使高保真验证变得可行，提供了可实现的运行性能实用上界，通过减少性能差距和计算成本，实现了更可靠的综合能源系统设计验证。
...
TL;DR: 提出ReDiRect框架，通过模糊分区将大规模交易图分解为可管理组件，在无监督设置下高效检测洗钱模式，减少误报并提升实用性。


<details>
  <summary>Details</summary>
Motivation: 现有洗钱检测方法存在局限性：洗钱者通过模仿正常交易模式逃避检测；现有算法难以处理大规模复杂交易图；基于风险规则的刚性系统产生过多误报信号；缺乏有效的无监督检测框架。
...
Result: 在真实开源Libra数据集和IBM Watson合成数据集上验证，ReDiRect框架在效率和实际应用性方面优于现有和最先进技术，实现了更优的性能表现。

Conclusion: ReDiRect框架通过创新的无监督交易图分区方法，有效解决了洗钱检测中的规模、复杂性和误报问题，为实际金融监控系统提供了更实用、高效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有模型合并方法存在理论缺陷：基于优化的方法需要数据估计协方差矩阵，而无需数据的启发式方法缺乏理论依据。需要一种既无需数据又有理论保证的合并方法

Method: 重新审视干扰最小化框架，证明在特定条件下可以直接从差异矩阵估计协方差矩阵，从而消除对数据的需求。该方法将模型合并转化为层级的优化问题，直接最小化任务间的干扰

Result: 在视觉和语言基准测试中验证了方法的有效性，模型规模从8600万参数到70亿参数不等，性能优于先前无需数据的最先进合并方法

Conclusion: 提出了一种既无需数据又有理论基础的模型合并方法，通过从差异矩阵估计协方差矩阵，实现了比现有数据无关方法更好的性能，为模型合并提供了更实用的解决方案
...
TL;DR: SECURE框架通过定义和执行模型鲁棒性，解决了深度学习中事故预测模型对微小输入扰动的不稳定性问题，在提升鲁棒性的同时提高了干净数据上的性能。


<details>
  <summary>Details</summary>
Motivation: 尽管深度学习显著推进了事故预测，但这些安全关键系统对现实世界扰动的鲁棒性仍然是一个重大挑战。研究发现即使像CRASH这样的最先进模型，在面对微小输入扰动时也会表现出显著的预测和潜在表示不稳定性，这带来了严重的可靠性风险。

Method: 提出了SECURE框架，基于四个关键属性：预测空间和潜在特征空间的一致性和稳定性。采用原则性的训练方法，通过多目标损失函数微调基线模型，该损失函数最小化与参考模型的差异，并惩罚对对抗性扰动的敏感性。

Result: 在DAD和CCD数据集上的实验表明，该方法不仅显著增强了针对各种扰动的鲁棒性，还提高了在干净数据上的性能，取得了新的最先进结果。

Conclusion: SECURE框架通过正式定义和执行模型鲁棒性，有效解决了事故预测模型对输入扰动的脆弱性问题，为安全关键系统提供了更可靠的解决方案。
...
TL;DR: 提出了一种基于并行前缀扫描的线性指数Hawkes过程最大似然估计算法，将计算复杂度从O(N²)降低到约O(N/P)，实现大规模并行化处理


<details>
  <summary>Details</summary>
Motivation: 传统多元Hawkes过程的最大似然估计计算复杂度为O(N²)，而现有的线性指数Hawkes过程虽然具有O(N)的递归计算，但都是顺序执行，无法充分利用现代GPU的并行计算能力

Method: 将Hawkes过程强度表示为稀疏转移矩阵的乘积，通过线性时间关联乘法实现并行前缀扫描计算，构建了简单且可大规模并行化的最大似然估计算法

Result: 在模拟和真实数据集上实现了数量级的加速，能够扩展到数千个节点和数千万个事件，远超先前工作的规模，同时保持恒定内存使用

Conclusion: 该方法为线性指数Hawkes过程提供了精确、高效且可扩展的最大似然估计解决方案，无需额外假设或近似，保持了模型的简单性和可解释性，并提供了开源实现
...
Method: 采用Malliavin微积分将反事实条件期望重新表述为无条件期望的比值，推导Langevin结构的Malliavin导数及其伴随Skorohod积分形式，设计具体算法进行反事实梯度估计

Result: 通过Malliavin方法克服了反事实梯度估计的困难，恢复了标准估计速率，避免了传统方法的低效问题

Conclusion: 提出的基于Malliavin微积分的被动Langevin算法为自适应逆强化学习提供了有效的反事实梯度估计框架，解决了传统方法收敛慢的问题
...
<details>
  <summary>Details</summary>
Motivation: 离线强化学习在无需与环境交互的情况下从先前收集的数据中学习策略，对高风险应用尤为重要。现有方法通常依赖严格的数据覆盖假设且易受分布偏移影响，需要更稳健的解决方案。

Method: 提出基于残差的离线RL框架，定义基于残差的Bellman最优算子，显式地将转移动态估计误差纳入策略优化过程。该算子利用经验残差，被证明是压缩映射，并在特定条件下具有渐近最优性和有限样本保证。进一步开发了基于残差的离线深度Q学习（DQN）算法。

Result: 在随机CartPole环境中验证了基于残差的离线DQN算法的有效性，证明了该方法能够处理分布偏移问题并提升离线强化学习的性能。

Conclusion: 基于残差的离线RL框架为处理一般状态和动作空间的离线强化学习问题提供了理论保证和实用算法，通过显式纳入动态估计误差增强了方法的稳健性，为解决离线RL中的分布偏移问题提供了新思路。
...
Method: 通过识别和列出六个与PPG信号相关的具体医学问题作为基准问题，同时描述适合的基准数据集及其使用规范。

Result: 建立了包含六个PPG相关医学问题的基准问题集，并提供了相应的基准数据集描述和使用指南。

Conclusion: 该报告为评估PPG信号处理中机器学习算法的不确定性提供了标准化的基准框架，有助于推动该领域的算法开发和性能比较。
...
### [22] [Test-Time Scaling Makes Overtraining Compute-Optimal](https://arxiv.org/abs/2604.01411)
...
Method: 提出T²缩放定律，将用于测试时缩放的pass@k建模整合到预训练缩放定律中，在固定端到端预算下联合优化模型大小、训练token数和推理样本数。通过任务损失和任务准确率两个维度验证方法的鲁棒性。

Result: 在八个下游任务上发现，当考虑推理成本时，最优预训练决策会大幅转向过训练区域，远超标准预训练缩放套件的范围。在T²预测的最优区域预训练过训练模型，验证了其相比仅预训练缩放的显著性能提升。研究结果在后训练阶段仍然有效。

Conclusion: T²缩放定律为现代LLM部署提供了更全面的成本效益分析框架，表明在考虑端到端推理成本时，过训练策略可能比传统预训练缩放定律建议的更优，这对实际部署具有重要指导意义。
...
TL;DR: 通过强化学习训练语言模型生成长思维链，利用测试时计算（"思考"）来改善权重内知识的潜在泛化能力


<details>
  <summary>Details</summary>
Motivation: 语言模型在权重内学习知识时，经常难以进行演绎推理（如反转诅咒问题），而上下文学习则表现出更强的潜在泛化能力。现有基于训练时数据增强的方法存在任务特定性、扩展性差和分布外泛化能力不足的问题。

Method: 使用基于正确性反馈的强化学习（RL）训练模型生成长思维链（CoTs），让模型在测试时进行"思考"以改善潜在泛化。这种方法不依赖训练时数据增强，而是教导模型利用测试时计算。

Result: "思考"方法不仅解决了分布内知识的潜在泛化失败问题，还能泛化到未进行RL训练的新知识。在纯反转任务中，思考虽不能直接实现知识反转，但通过生成-验证能力使模型表现显著高于随机水平。不过，事实自验证的脆弱性使思考模型在该任务上仍远低于上下文学习的性能。

Conclusion: 测试时思考是改善语言模型潜在泛化能力的灵活且有前景的方向，特别是对于权重内学习知识，但仍有改进空间。
...
### [24] [Soft MPCritic: Amortized Model Predictive Value Iteration](https://arxiv.org/abs/2604.01477)
*Thomas Banker,Nathan P. Lawrence,Ali Mesbah*

Main category: cs.LG

TL;DR: 软MPCritic是一种结合强化学习与模型预测控制的框架，通过在软价值空间中学习，同时使用基于样本的规划进行在线控制和价值目标生成，实现计算效率与性能的平衡。
...
Result: 软MPCritic在经典和复杂控制任务中通过鲁棒的短时域规划有效学习，在保持解质量的同时实现计算可行性，为在策略提取和直接长时域规划可能失败的场景中合成MPC策略提供了实用蓝图。

Conclusion: 软MPCritic为结合RL和MPC提供了一个实用且可扩展的框架，通过软价值空间学习与基于样本的规划相结合，解决了计算挑战，同时保持了解决方案的质量，适用于复杂控制任务。
...
### [25] [Transformer self-attention encoder-decoder with multimodal deep learning for response time series forecasting and digital twin support in wind structural health monitoring](https://arxiv.org/abs/2604.01712)
*Feiyu Zhou,Marios Impraimakis*

Main category: cs.LG

TL;DR: 本文提出了一种基于Transformer的桥梁结构健康监测数字孪生框架，用于风致结构响应预测和早期预警，在Hardanger大桥实测数据上验证了其优于传统方法的性能。


<details>
  <summary>Details</summary>
Motivation: 风激动态行为存在不确定性，当环境或交通条件变化时传统方法预测效果不佳，难以区分正常振动行为。需要一种不依赖风平稳性或结构正常振动假设的预测方法。

Method: 采用Transformer方法构建数字孪生组件：1) 利用系统时间特征训练预测模型；2) 将振动预测值与实测值比较检测大偏差；3) 将识别出的异常情况作为结构变化的早期预警指标。

Result: 基于人工智能的模型在响应预测方面优于传统方法，不需要风平稳性或结构正常振动行为的假设。在挪威科技大学监测的Hardanger大桥实际测量数据上验证，该方法能准确捕捉真实条件下的结构行为，适应系统激励的变化。

Conclusion: Transformer基数字孪生组件有潜力成为下一代韧性基础设施管理工具，支持连续学习和自适应监测，适应系统生命周期内的时间特征变化。
...
TL;DR: ES与GRPO在LLM微调中达到相似任务性能，但产生几何结构不同的参数空间解，ES更新更大、KL漂移更广，而GRPO更新更局部化，两者解线性连接无损失障碍


<details>
  <summary>Details</summary>
Motivation: 研究进化策略（ES）与基于梯度的强化学习方法（GRPO）在LLM微调中是否产生相似的任务性能意味着在参数空间中产生相似的解，探索两种方法在单任务和持续学习设置中的差异

Method: 在四个任务上比较ES和GRPO，包括单任务和顺序持续学习设置，控制ES的迭代预算，分析模型更新大小、KL漂移、解空间几何结构，并开发ES分析理论解释观察到的现象

Result: ES在单任务准确率上匹配或超过GRPO，在顺序学习中保持竞争力；ES产生更大的模型更新和更广泛的离任务KL漂移，GRPO更新更小更局部；两种方法的解线性连接且无损失障碍，尽管更新方向几乎正交

Conclusion: 梯度自由和基于梯度的微调方法可以达到相似准确率但几何结构不同的解，这对遗忘和知识保留有重要影响，ES可以在弱信息方向上积累大的离任务移动，同时在任务上取得足够进展
...
TL;DR: 论文提出REPAIR方法，通过贝叶斯最优重排序解决长尾分类问题，将残差校正分解为类别恒定项和输入依赖的成对项，当同一标签对在不同上下文中产生矛盾排序约束时，固定偏移无法恢复最优排序。


<details>
  <summary>Details</summary>
Motivation: 长尾分类中模型偏向频繁类别，现有后处理方法使用固定类别偏移调整logits，但最优校正可能随输入变化，固定偏移无法适应这种变化，需要更灵活的校正机制。

Method: 提出REPAIR方法：基于基础模型top-k候选列表进行贝叶斯最优重排序，将残差校正分解为类别恒定项和线性成对项，后者由候选列表中的竞争特征驱动，使用收缩稳定化技术。

Result: 在图像分类、物种识别、场景识别和罕见疾病诊断五个基准测试中验证了分解的有效性，解释了成对校正何时有效以及何时仅类别校正就足够。

Conclusion: 残差校正的分解为理解长尾分类中校正机制提供了理论框架，REPAIR方法通过结合类别恒定项和输入依赖的成对项，在需要时提供更灵活的校正，提高了长尾分类性能。
...
TL;DR: ECG-Scan：一种通过双生理感知对齐从心电图图像中学习临床通用表示的自监督框架，显著缩小了心电图图像分析与信号分析之间的差距。
...
Method: 提出ECG-Scan自监督框架，通过双生理感知对齐：1）使用图像与金标准信号-文本模态之间的多模态对比对齐优化图像表示学习；2）通过软导联约束整合领域知识，正则化重建过程并改善信号导联间一致性。
...
<details>
  <summary>Details</summary>
Motivation: 当前大语言模型的对齐方法主要优化帮助性和无害性，但忽视了关系风险（如依赖强化、过度保护、强制指导等），这些风险会损害用户自主性。需要开发能平衡帮助性和自主性保护的对话系统。

Method: 提出Care-Conditioned Neuromodulation (CCN)框架：1) 从结构化用户状态和对话上下文学习标量信号；2) 将该信号作为状态依赖控制来调节响应生成和候选选择；3) 形式化为自主性保护对齐问题，定义奖励自主性支持和帮助性、惩罚依赖和强制的效用函数；4) 结合基于效用的重排序。

Result: 在构建的关系失败模式基准测试中，CCN结合效用重排序比监督微调提升自主性保护效用+0.25，比偏好优化基线提升+0.07，同时保持相当的帮助性。初步人工评估和零样本迁移到真实情感支持对话显示与自动指标方向一致。

Conclusion: 状态依赖控制结合基于效用的选择是处理自主性敏感对话中多目标对齐的实用方法，能在保持帮助性的同时更好地保护用户自主性。
...
TL;DR: 将潜在循环建模扩展到序列输入流，通过交错快速潜在更新和慢速观测更新，学习随输入演化的稳定内部结构，提升长时程表示一致性和泛化能力


<details>
  <summary>Details</summary>
Motivation: 现有序列模型（如LSTM、状态空间模型、Transformer变体）在长时程表示一致性和分布外泛化方面存在不足，需要一种能够学习稳定内部结构并随输入演化的方法

Method: 在慢速观测更新之间交错快速循环潜在更新，利用自组织能力学习演化中的稳定内部结构，保持表示的一致性和聚类性

Result: 相比LSTM、状态空间模型和Transformer变体等序列基线，在强化学习和算法任务中表现出更好的长时程表示一致性和分布外泛化能力

Conclusion: 通过交错不同时间尺度的更新机制，能够有效学习稳定且演化的内部表示结构，为序列建模提供了新的有效方法
...
### [31] [Learning from the Right Rollouts: Data Attribution for PPO-based LLM Post-Training](https://arxiv.org/abs/2604.01597)
*Dong Shu,Denghui Zhang,Jessica Hullman*

Main category: cs.LG

TL;DR: I-PPO通过梯度影响评分筛选训练数据，消除反对齐的推理轨迹，提升PPO训练效率和忠实性


<details>
  <summary>Details</summary>
Motivation: 传统PPO算法假设所有生成轨迹都提供有益优化信号，但实际上许多轨迹包含噪声或不忠实的推理，这会降低模型性能并减慢训练速度

Method: 提出Influence-Guided PPO (I-PPO)框架，在RL后训练循环中集成数据归因，通过基于梯度的近似计算每个轨迹的影响分数，识别并消除与验证梯度反对齐的轨迹

Result: 实验表明I-PPO持续优于SFT和PPO基线，过滤过程起到内在早停机制作用，加速训练效率，有效减少不忠实的思维链推理

Conclusion: I-PPO通过选择性使用高质量训练数据，解决了传统RL算法中噪声轨迹对训练的负面影响，为提升语言模型对齐训练提供了有效方法
...
### [32] [Training In-Context and In-Weights Mixtures Via Contrastive Context Sampling](https://arxiv.org/abs/2604.01601)
*Deeptanshu Malu,Deevyanshu Malu,Aditya Nemiwal,Sunita Sarawagi*

Main category: cs.LG

TL;DR: 论文研究了上下文学习(ICL)和权重学习(IWL)的协同训练策略，提出对比上下文方法来解决随机上下文导致ICL丢失和相似上下文导致标签复制的问题。


<details>
  <summary>Details</summary>
Motivation: 当前LLMs虽然同时具备ICL和IWL能力，但标准任务特定微调往往会侵蚀ICL能力。IC-Train（使用上下文示例进行微调）的先前研究表明，ICL的出现取决于任务多样性和训练时长等因素，但输入目标与上下文示例之间的相似性结构的作用尚未充分研究。

Method: 提出对比上下文方法，包含两种对比：(1) 在单个上下文中混合相似和随机示例，以演化出正确的ICL形式；(2) 在不同上下文中使用不同程度的相似性，以演化ICL-IWL混合。通过理论分析最小模型验证对比的重要性，并在四个LLM和多个任务上进行实证评估。

Result: 随机上下文导致ICL丢失和IWL主导，而仅包含相似示例的上下文导致ICL退化为不考虑相关性的标签复制。对比上下文方法能够产生稳定的ICL-IWL混合，避免纯ICL、纯IWL或标签复制的崩溃。诊断探针证实了这种稳定性。

Conclusion: 目标输入与上下文示例之间的相似性结构对ICL-IWL协同发展至关重要。提出的对比上下文方法通过强制两种类型的对比，能够有效演化出正确的ICL形式和稳定的ICL-IWL混合，避免训练过程中的退化现象。
...
### [33] [Expert-Choice Routing Enables Adaptive Computation in Diffusion Language Models](https://arxiv.org/abs/2604.01622)
...
TL;DR: 本文提出将专家选择路由应用于扩散语言模型，通过时间步相关的专家容量分配策略，实现了更好的负载均衡和计算效率，显著提升了模型性能。
...
Method: 采用专家选择路由替代令牌选择路由，引入时间步相关的专家容量分配策略，根据去噪步骤动态调整专家分配。具体来说，在低掩码率步骤分配更多容量，并展示了如何将预训练的令牌选择路由模型改造为专家选择路由。

Result: 专家选择路由实现了确定性的负载均衡，相比令牌选择路由获得了更高的吞吐量和更快的收敛速度。在匹配FLOPs条件下，低掩码率步骤分配更多容量的策略始终获得最佳性能。预训练模型仅通过替换路由器即可改造为专家选择路由，在多样化下游任务中实现了更快的收敛和更高的准确性。
...
### [34] [CRIT: Graph-Based Automatic Data Synthesis to Enhance Cross-Modal Multi-Hop Reasoning](https://arxiv.org/abs/2604.01634)
*Junyoung Sung,Seungwoo Lyu,Minjun Kim,Sumin An,Arsha Nagrani,Paul Hongsuck Seo*

Main category: cs.LG

TL;DR: CRIT是一个新的跨模态推理数据集和基准测试，通过图基自动管道生成复杂的跨模态多跳推理任务，解决现有VLMs在结合文本和视觉信息进行多步推理方面的不足。


<details>
  <summary>Details</summary>
Motivation: 现实世界推理需要跨模态结合信息，在文本和视觉线索之间进行多跳推理。然而，现有多模态基准测试通常依赖单张图像或图像集，答案可以从单一模态推断，训练数据中的交错图像-文本内容也很少强制互补的多跳推理，导致VLMs经常产生幻觉，推理轨迹缺乏视觉证据支撑。

Method: 引入CRIT数据集和基准测试，采用图基自动管道生成复杂的跨模态推理任务。数据集涵盖自然图像、视频和文本丰富来源等多个领域，并包含手动验证的测试集以确保可靠评估。

Result: 实验表明，即使是当前最先进的模型在这类推理任务上也表现不佳。在CRIT上训练的模型在跨模态多跳推理方面取得显著提升，包括在SPIQA和其他标准多模态基准测试上的明显改进。

Conclusion: CRIT填补了现有多模态基准测试的空白，为评估和提升VLMs的跨模态多跳推理能力提供了有效工具，展示了专门训练数据对改善模型推理能力的重要性。
...
### [35] [Label Shift Estimation With Incremental Prior Update](https://arxiv.org/abs/2604.01651)
*Yunrui Zhang,Gustavo Batista,Salil S. Kanhere*

Main category: cs.LG

TL;DR: 提出一种后验标签分布估计新方法，通过逐样本更新先验来调整后验，相比现有方法基于更弱的校准要求，适用于任何黑盒概率分类器，在CIFAR-10和MNIST上优于当前最先进的极大似然方法。


<details>
  <summary>Details</summary>
Motivation: 监督学习中训练集和测试集具有相同标签分布的假设在现实中很少成立（如医疗诊断、欺诈检测、社交媒体分类等场景），标签分布会随时间、地点、趋势等变化。现有标签偏移估计方法存在局限性，需要更灵活有效的解决方案。

Method: 提出一种后验标签偏移估计方法，通过逐样本增量更新先验分布来调整后验概率，基于对现代概率分类器的直观假设，相比其他方法仅需更弱的校准概念。该方法作为后处理技术，可应用于任何黑盒概率分类器。

Result: 在CIFAR-10和MNIST数据集上的实验表明，该方法在不同校准程度和不同强度标签偏移下，始终优于当前最先进的基于极大似然的方法。

Conclusion: 提出的后验标签偏移估计方法具有更好的性能、更弱的校准要求、更强的通用性，能够有效应对现实场景中的标签分布变化问题。
...
### [36] [Cognitive Energy Modeling for Neuroadaptive Human-Machine Systems using EEG and WGAN-GP](https://arxiv.org/abs/2604.01653)
*Sriram Sattiraju,Vaibhav Gollapalli,Aryan Shah,Timothy McMahan*

Main category: cs.LG

TL;DR: 该研究使用薛定谔桥问题（SBP）的传输成本作为度量，评估GAN生成的合成EEG是否保留了认知状态转换所需的分布几何结构，并证明合成EEG保留了SBP建模所需的转换结构。


<details>
  <summary>Details</summary>
Motivation: 虽然GAN等生成模型已广泛用于增强EEG数据，但尚不清楚合成EEG是否保留了基于转换分析所需的底层动态结构。本研究旨在填补这一空白，评估合成EEG是否保留了基于能量的认知状态转换建模所需的分布几何。

Method: 使用薛定谔桥问题（SBP）衍生的传输成本作为度量，比较从真实和合成EEG（在Stroop任务中收集）中推导出的转换能量。在群体和参与者层面进行分析，评估合成EEG是否保留了SBP建模所需的转换结构。

Result: 在Stroop任务中，真实和合成EEG推导出的转换能量在群体和参与者层面分析中表现出强烈一致性。这表明合成EEG保留了SBP建模所需的转换结构。

Conclusion: 合成EEG保留了SBP建模所需的转换结构，使其能够在数据高效的神经自适应系统中使用。研究进一步提出了一个框架，其中SBP衍生的认知能量作为自适应人机系统的控制信号，支持系统行为根据用户认知和情感状态进行实时调整。
...
### [37] [MiCA Learns More Knowledge Than LoRA and Full Fine-Tuning](https://arxiv.org/abs/2604.01694)
*Sten Rüdiger,Sebastian Raschka*

Main category: cs.LG

TL;DR: MiCA是一种新颖的参数高效微调方法，专注于适配模型表示中未充分利用的子空间，通过奇异值分解识别与最小奇异值相关的次要奇异向量方向，相比LoRA在知识获取方面提升5.9倍，参数占用仅为6-60%。


<details>
  <summary>Details</summary>
Motivation: 传统参数高效微调方法如LoRA主要针对模型表示中的主导子空间，但作者认为适配未充分利用的次要子空间可能提供更高效、更稳定的知识整合机制。

Method: 使用奇异值分解识别与最小奇异值相关的次要奇异向量方向，在微调过程中将参数更新约束在这些次要方向上，从而适配模型表示中未充分利用的子空间。

Result: 在优化的训练超参数下，MiCA在知识获取方面相比LoRA提升高达5.9倍，同时参数占用仅为LoRA的6-60%，表明次要方向适配提供了更高效的知识整合。

Conclusion: 将适配约束在次要奇异方向上为预训练语言模型整合新知识提供了更高效、更稳定的机制，相比传统主导子空间适配方法具有显著优势。
...
### [38] [MATA-Former & SIICU: Semantic Aware Temporal Alignment for High-Fidelity ICU Risk Prediction](https://arxiv.org/abs/2604.01727)
*Zhichong Zheng,Xiaohang Nie,Xueqi Wang,Yuanjin Zhao,Haitao Zhang,Yichao Tang*

Main category: cs.LG

TL;DR: 提出MATA-Former框架，利用事件语义动态参数化注意力权重，优先考虑因果有效性而非时间滞后，并引入Plateau-Gaussian软标签将二元分类转化为连续多时间范围回归，用于临床风险预测。


<details>
  <summary>Details</summary>
Motivation: 当前临床风险预测方法存在两个主要问题：1) 依赖粗糙的二元监督和物理时间戳，而非内在病理依赖关系；2) 预测建模与临床逻辑不一致。需要开发能更好捕捉临床事件语义依赖的模型。

Method: 1) 提出Medical-semantics Aware Time-ALiBi Transformer (MATA-Former)，利用事件语义动态参数化注意力权重，优先考虑因果有效性而非时间滞后；2) 引入Plateau-Gaussian Soft Labeling (PSL)，将二元分类重新表述为连续多时间范围回归，实现全轨迹风险建模；3) 构建SIICU数据集（包含50.6万+事件，具有专家验证的细粒度标注）。

Result: 在SIICU（新构建数据集）和MIMIC-IV数据集上评估，该框架在捕捉文本密集、不规则临床时间序列风险方面表现出优越的效能和稳健的泛化能力。

Conclusion: 通过将事件语义融入注意力机制和采用连续风险建模方法，MATA-Former框架成功地将预测建模与临床逻辑对齐，显著提升了临床风险预测的准确性和可解释性。
...
### [39] [Koopman-Based Nonlinear Identification and Adaptive Control of a Turbofan Engine](https://arxiv.org/abs/2604.01730)
*David Grasev*

Main category: cs.LG

TL;DR: 该研究开发了基于Koopman算子的双转子涡扇发动机多变量控制方法，提出元启发式扩展动态模态分解构建适用于多种控制目标的单一Koopman模型，并比较了自适应Koopman模型预测控制器与反馈线性化控制器的性能。


<details>
  <summary>Details</summary>
Motivation: 研究动机是开发适用于涡扇发动机多变量控制的Koopman算子方法，解决传统控制方法在处理非线性系统时的局限性，实现单一模型适应多种控制目标（转子转速和发动机压力比），并提升在变化飞行条件下的鲁棒性。

Method: 方法包括：1）建立基于物理的部件级模型生成训练数据；2）开发元启发式扩展动态模态分解方法，设计成本函数准确捕捉转子转速动态和发动机压力比；3）构建时变Koopman模型；4）开发自适应Koopman模型预测控制器（带扰动观测器）和Koopman反馈线性化控制器；5）在海平面和变化飞行条件下评估两种控制策略（转子转速配置和EPR配置）。

Result: 结果表明：1）提出的辨识方法能准确预测转子转速和EPR，使Koopman模型可在不同控制框架中灵活重用；2）两种控制策略在稳态条件下性能相当；3）在变化飞行条件下，AKMPC因能补偿模型失配而表现出优于K-FBLC的鲁棒性；4）EPR控制策略改善了推力响应。

Conclusion: 研究证实了Koopman基控制在涡扇发动机控制中的适用性，展示了AKMPC框架在鲁棒控制方面的优势，为非线性航空发动机控制提供了有效的基于数据驱动的方法论。
...
### [40] [FourierMoE: Fourier Mixture-of-Experts Adaptation of Large Language Models](https://arxiv.org/abs/2604.01762)
*Juyong Jiang,Fan Wang,Hong Qi,Sunghun Kim,Jing Tang*

Main category: cs.LG

TL;DR: FourierMoE：一种在频域进行参数高效微调的新方法，通过将MoE架构与逆离散傅里叶变换结合，实现频率感知的专家适配，在减少参数的同时提升多任务微调性能。


<details>
  <summary>Details</summary>
Motivation: 现有参数高效微调方法在多任务设置中存在任务干扰和表示不足问题，而基于空间域的MoE方法存在结构冗余和参数开销。研究发现不同任务具有不同的频率能量分布，LLM层表现出异质的频率敏感性，这为频域适配提供了理论基础。

Method: 提出FourierMoE，将MoE架构与逆离散傅里叶变换结合：1）使用频率自适应路由器将token分发到不同频率带的专家；2）每个专家学习一组共轭对称的复数系数，保留完整的相位和幅度信息；3）通过IDFT理论保证无损重建为实值空间权重。

Result: 在28个基准测试、多种模型架构和规模上的广泛评估表明，FourierMoE在单任务和多任务设置中均优于竞争基线，同时使用显著更少的可训练参数。

Conclusion: 频域专家适配是LLM微调的一种有效且参数高效的范式，FourierMoE通过频率感知的适配机制解决了多任务微调中的任务干扰和表示不足问题。
...
### [41] [Bridging Deep Learning and Integer Linear Programming: A Predictive-to-Prescriptive Framework for Supply Chain Analytics](https://arxiv.org/abs/2604.01775)
*Khai Banh Nghiep,Duc Nguyen Minh,Lan Hoang Thi*

Main category: cs.LG

TL;DR: 该研究提出一个结合预测与运营分析的三步框架，用于解决零售需求预测中的季节性、不规则峰值和噪声问题，最终通过优化模型生成成本最优的配送计划。


<details>
  <summary>Details</summary>
Motivation: 零售数据常呈现不可调和的季节性、不规则峰值和噪声，使得精确预测几乎无法实现，这对供应链规划至关重要。需要开发一个结合预测和运营分析的框架来解决这一挑战。

Method: 1. 探索性数据分析：对180,519笔交易数据进行分区，分析长期趋势、季节性和配送相关属性；2. 比较N-BEATS MSTL统计时间序列分解模型与N-HiTS深度学习架构的预测性能；3. 选择最优模型（N-BEATS）预测未来4周需求，结合整数线性规划优化配送计划，在预算、容量和服务约束下最小化总配送时间。

Result: N-HiTS和N-BEATS深度学习模型在预测性能上大幅超越统计基准。N-BEATS被选为最优模型（最低预测误差），预测未来4周需求为1918单位。优化后的整数线性规划方案提供了可行且成本最优的配送计划。
...
### [42] [Graph Neural Operator Towards Edge Deployability and Portability for Sparse-to-Dense, Real-Time Virtual Sensing on Irregular Grids](https://arxiv.org/abs/2604.01802)
...
<details>
  <summary>Details</summary>
Motivation: 现实世界中密集仪器部署成本高、不可行，而传统物理求解器计算延迟和功耗大，无法在资源受限的边缘系统中实时使用。需要一种能在边缘设备上实时运行的高效虚拟传感方法。

Method: 提出VIRSO（虚拟不规则实时稀疏算子），一种基于图的神经算子，用于不规则几何上的稀疏到密集重建。开发了可变连接算法V-KNN，用于构建网格感知图。将推理重构为测量过程，结合频谱和空间分析，实现准确重建。

Result: 在三个核热工水力基准测试中，重建比从47:1到156:1，平均相对L2误差低于1%，优于其他基准算子且参数更少。10层配置将能量延迟积从约206J·ms降至10.1J·ms（NVIDIA H200）。在Jetson Orin Nano上，所有配置功耗低于10W，延迟低于1秒。
...
<details>
  <summary>Details</summary>
Motivation: 现实部署中分布偏移普遍存在，导致预训练异常检测器性能严重下降。测试时自适应（TTA）仅使用未标记测试数据在线更新预训练模型，为解决这一挑战提供了可能。

Method: 提出CANDI框架，包含：1）假阳性挖掘（FPM）策略，基于异常分数和潜在相似性筛选适应样本；2）即插即用的时空感知正态性适应（SANA）模块，进行结构化模型更新。

Result: 大量实验表明CANDI显著提升了分布偏移下MTSAD性能，AUROC提升高达14%，同时使用更少的适应样本。
...
TL;DR: 提出基于扩散的后验采样框架，为工业过程监控中的数据驱动模型提供可靠的不确定性量化，无需后处理校准
...
TL;DR: ASPECT框架通过可靠性感知的频谱门控机制解决谱图对比学习中的频谱困境，在混合图上实现节点级最优频谱融合，在9个基准中的8个达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有谱图对比学习方法在处理同质性和异质性混合图时面临频谱困境：高频信号对编码异质性至关重要，但理论分析表明其在频谱集中扰动下具有更高方差。全局频谱融合策略在节点级频率偏好分离的混合图上存在不可消除的遗憾。

Method: 提出ASPECT框架，通过可靠性感知的频谱门控机制解决频谱困境。该框架采用极小极大博弈设计：节点级门控根据频谱通道对专门构建的对抗者的稳定性动态重新加权频率通道，对抗者通过Rayleigh商惩罚显式针对频谱能量分布，迫使编码器学习既具有结构判别性又具有频谱鲁棒性的表示。

Result: 在9个基准测试中的8个上实现了新的最先进性能，有效解耦了有意义的结构异质性与偶然噪声。

Conclusion: ASPECT通过节点级可靠性感知的频谱门控机制成功解决了谱图对比学习中的频谱困境，为处理混合图提供了一种既保持结构判别性又具有频谱鲁棒性的统一框架。
...
### [46] [Enhancing the Reliability of Medical AI through Expert-guided Uncertainty Modeling](https://arxiv.org/abs/2604.01898)
*Aleksei Khalin,Ekaterina Zaychenkova,Aleksandr Yugay,Andrey Goncharov,Sergey Korchagin,Alexey Zaytsev,Egor Ershov*

Main category: cs.LG

TL;DR: 提出一种利用专家意见分歧来改进医疗AI不确定性估计的新方法，通过双集成模型分别量化认知和偶然不确定性，在多个医疗任务中显著提升不确定性估计质量。


<details>
  <summary>Details</summary>
Motivation: 医疗AI系统作为第二意见系统可加速工作流程并提高诊断准确性，但其错误的不可预测性带来重大挑战，尤其是在医疗环境中错误可能造成严重后果。现有不确定性估计方法在量化偶然不确定性（源于数据模糊和噪声）方面存在局限。

Method: 提出利用专家响应中的分歧来生成训练机器学习模型的目标，这些目标与标准数据标签结合使用，通过双集成方法（及其轻量变体）根据全方差定律分别估计不确定性的两个组成部分。

Result: 在二值图像分类、二值和多类图像分割以及多项选择题回答等任务上验证了该方法。实验表明，结合专家知识可将不确定性估计质量提高9%到50%（取决于具体任务）。
...
### [47] [annbatch unlocks terabyte-scale training of biological data in anndata](https://arxiv.org/abs/2604.01949)
...
Method: 开发annbatch作为anndata的原生mini-batch加载器，支持直接在磁盘数据集上进行内存外训练，保持与scverse生态系统的完全兼容性
...
### [48] [Learn by Surprise, Commit by Proof](https://arxiv.org/abs/2604.01951)
*Kang-Sin Choi*
...
<details>
  <summary>Details</summary>
Motivation: 解决模型在标准微调中出现的机械记忆问题，实现语义学习而非表面记忆；通过自主知识获取减少幻觉，特别是针对弱编码的现有知识；模拟生物记忆巩固过程，将上下文中的临时信息选择性地整合到参数权重中。

Method: 1) 检测异常高损失段落；2) 生成问答链让模型自我验证知识缺口；3) 基于验证深度k自适应调整AdamW的β₂参数：β₂ = 0.999·r^k；4) 整个学习强度由单个参数r控制；5) 框架具有自熄灭特性，随着学习进行逐渐收敛到标准AdamW。

Result: 在Qwen3-14B和六个模型(8B-32B，四个家族)上的实验表明：标准微调产生机械记忆(扰动间隙11.6±0.2倍基线)，而所有LSCP条件实现语义学习(2.7-3.0倍)。r=1.0条件证明训练数据格式(而非β₂门控)是防止记忆的主要机制；门控机制保护相邻知识免受污染内容影响(相邻问题准确率93±7% vs 90%基线)。

Conclusion: LSCP框架通过自我验证和自适应学习强度，实现了自主知识获取和语义学习，有效防止机械记忆并保护现有知识。该框架模拟了生物记忆巩固过程，为大型语言模型的后训练提供了新的自监督学习范式。
...
### [49] [World Action Verifier: Self-Improving World Models via Forward-Inverse Asymmetry](https://arxiv.org/abs/2604.01985)
*Yuejiang Liu,Fan Feng,Lingjing Kong,Weifeng Lu,Jinzhou Tang,Kun Zhang,Kevin Murphy,Chelsea Finn,Yilun Du*

Main category: cs.LG

TL;DR: WAV框架通过将状态预测分解为状态合理性和动作可达性两个因素，利用动作无关数据的广泛可用性和动作相关特征的低维度性，使世界模型能够自我验证预测错误并自我改进。


<details>
  <summary>Details</summary>
Motivation: 通用世界模型需要比策略学习更广泛的鲁棒性，必须对大量次优动作保持可靠预测，而这些动作在带标签的交互数据中通常覆盖不足。现有方法在未充分探索的区域容易失败。

Method: 提出WAV框架：1) 将动作条件状态预测分解为状态合理性和动作可达性；2) 利用视频语料库生成多样化子目标；3) 使用稀疏逆模型从状态特征子集推断动作；4) 通过生成子目标、推断动作和前向推演的循环一致性实现验证机制。
...
### [50] [Apriel-Reasoner: RL Post-Training for General-Purpose and Efficient Reasoning](https://arxiv.org/abs/2604.02007)
*Rafael Pardinas,Ehsan Kamalloo,David Vazquez,Alexandre Drouin*

Main category: cs.LG

TL;DR: Apriel-Reasoner是一个15B参数的开源LLM，通过多领域RL后训练在数学、代码生成、指令遵循、逻辑谜题和函数调用五个领域进行优化，采用自适应领域采样和难度感知长度惩罚机制，在保持输出质量的同时减少30-50%的推理轨迹长度。


<details>
  <summary>Details</summary>
Motivation: 现有通用推理模型通常不公开训练配方和领域混合策略，且跨领域联合优化面临挑战：不同领域在轨迹长度、问题难度和样本效率上差异巨大。长思维链会增加推理成本和延迟，因此效率对实际部署至关重要。

Method: 1) 在Apriel-Base 15B参数模型上进行多领域RL后训练；2) 引入自适应领域采样机制，在异质轨迹动态下保持目标领域比例；3) 提出难度感知的标准长度惩罚扩展，无需额外训练开销，鼓励困难问题使用更长推理，简单问题使用更短轨迹；4) 训练时严格限制16K令牌输出预算，推理时扩展到32K令牌。

Result: Apriel-Reasoner在AIME 2025、GPQA、MMLU-Pro和LiveCodeBench上优于Apriel-Base，同时产生30-50%更短的推理轨迹。在相似规模的开源模型中达到同等性能但使用更少令牌，推动了准确率与令牌预算的帕累托前沿。

Conclusion: Apriel-Reasoner通过可复现的多领域RL后训练配方，结合自适应领域采样和难度感知长度惩罚，实现了在保持或提升性能的同时显著减少推理成本，为高效推理模型提供了实用解决方案。
...
### [51] [Feature Weighting Improves Pool-Based Sequential Active Learning for Regression](https://arxiv.org/abs/2604.02019)
*Dongrui Wu*

Main category: cs.LG

TL;DR: 提出基于特征加权的主动学习回归方法，通过使用岭回归系数对特征进行加权，改进样本间距离计算，从而优化样本选择


<details>
  <summary>Details</summary>
Motivation: 传统主动学习回归方法在计算样本间距离时未考虑不同特征的重要性，导致样本选择次优。特征加权能更好地捕捉样本的代表性和多样性

Method: 提出三种特征加权单任务ALR方法和两种特征加权多任务ALR方法。利用少量已标注样本训练的岭回归系数对相应特征进行加权，改进样本间距离计算

Result: 实验表明这种易于实现的增强方法几乎总能提升四种现有ALR方法的性能，适用于单任务和多任务回归问题

Conclusion: 特征加权策略能有效改进主动学习回归的样本选择，且易于扩展到流式主动学习和分类算法
...
### [52] [AA-SVD : Anchored and Adaptive SVD for Large Language Model Compression](https://arxiv.org/abs/2604.02119)
*Atul Kumar Sinha,François Fleuret*

Main category: cs.LG

TL;DR: 提出一种基于低秩分解的快速大语言模型压缩框架，通过同时考虑原始输入和压缩引起的分布偏移，实现无需重训练的快速压缩，并在端到端块级优化中减少累积误差。


<details>
  <summary>Details</summary>
Motivation: 现有基于分解的压缩方法存在两个主要问题：1）仅优化原始输入，忽略上游压缩引起的分布偏移，导致误差传播；2）仅依赖偏移输入，可能偏离原始输出。需要一种能同时考虑两者的方法，以在保持功能等效的同时实现高效压缩。

Method: 提出一个低秩分解框架，通过将每个压缩层锚定到原始输出，同时显式建模输入分布偏移，找到与原始模型功能等效的低秩近似。除了单层压缩外，进一步对每个Transformer块进行端到端细化，最小化块级输出失真，允许压缩层共同补偿累积误差。

Result: 在大语言模型上的实验表明，该方法在各种压缩比下始终优于现有的基于SVD的基线方法。在激进压缩预算下优势尤为明显，而竞争方法则显著退化或完全失效，为高效、大规模模型部署提供了实用解决方案。

Conclusion: 该方法通过同时考虑原始输入和分布偏移，结合块级端到端优化，实现了无需重训练的高效大语言模型压缩，在保持功能等效的同时显著优于现有方法，特别是在高压缩比下表现优异。
...
### [53] [Auction-Based Online Policy Adaptation for Evolving Objectives](https://arxiv.org/abs/2604.02151)
*Guruprerana Shabadi,Kaushik Mallik*

Main category: cs.LG

TL;DR: 提出一种基于拍卖机制的模块化多目标强化学习框架，通过自私的局部策略竞争执行权，实现运行时目标变化的动态适应。
...
Method: 采用模块化框架：每个目标对应一个自私的局部策略，通过拍卖机制协调——策略根据当前状态紧急程度出价，最高出价者选择动作。将问题转化为一般和博弈，策略使用PPO并发训练。

Result: 在Atari Assault和网格世界路径规划任务上评估，相比使用PPO训练的单一策略，该方法性能显著更好。

Conclusion: 提出的拍卖机制模块化框架能有效处理动态多目标强化学习问题，实现运行时目标变化的即时适应，性能优于传统单一策略方法。
...
### [54] [Smoothing the Landscape: Causal Structure Learning via Diffusion Denoising Objectives](https://arxiv.org/abs/2604.02250)
*Hao Zhu,Di Zhou,Donna Slonim*

Main category: cs.LG

TL;DR: DDCD利用扩散模型的去噪分数匹配目标来平滑梯度，实现更快更稳定的收敛，并提出自适应k跳无环约束提高运行效率，用于从观测数据中发现因果结构。
...
### [55] [Model-Based Reinforcement Learning for Control under Time-Varying Dynamics](https://arxiv.org/abs/2604.02260)
*Klemens Iten,Bruce Lee,Chenhao Li,Lenart Treven,Andreas Krause,Bhavya Sukhija*

Main category: cs.LG

TL;DR: 研究在时变动力学下的强化学习控制，提出带自适应数据缓冲机制的乐观模型基RL算法，在非平稳动力学连续控制基准上表现更优


<details>
  <summary>Details</summary>
Motivation: 现实世界系统通常存在动力学非平稳性（漂移、磨损、运行条件变化），而传统基于学习的控制方法假设平稳动力学，这在实际应用中往往不成立。需要研究在时变动力学下的强化学习控制问题。

Method: 采用持续模型基强化学习框架，使用高斯过程动力学模型，在频率派变分预算假设下分析问题。提出带自适应数据缓冲机制的乐观模型基强化学习算法，通过限制过时数据的影响来维持校准的不确定性和有意义的动态遗憾保证。

Result: 分析表明持续性非平稳性需要明确限制过时数据的影响。提出的算法在具有非平稳动力学的连续控制基准测试中表现出改进的性能。

Conclusion: 处理时变动力学需要专门设计的数据管理策略，自适应数据缓冲机制是维持模型校准和保证性能的关键，为现实世界非平稳系统的强化学习控制提供了实用解决方案。
...
TL;DR: SRPO是一种统一的在线策略优化框架，结合了GRPO的奖励对齐强化和SDPO的针对性对数级修正，通过样本路由和熵感知动态加权机制解决了SDPO的后期训练不稳定性问题。


<details>
  <summary>Details</summary>
Motivation: 现有RLVR方法存在局限性：GRPO采用粗粒度的信用分配，对失败rollout进行统一惩罚，缺乏针对特定偏差的token级关注；SDPO虽然提供更密集、更有针对性的对数级监督，能够实现快速早期改进，但在长期训练中经常崩溃。需要解决SDPO后期不稳定的内在缺陷。

Method: 提出样本路由策略优化（SRPO）：1）将正确样本路由到GRPO的奖励对齐强化学习；2）将失败样本路由到SDPO的针对性对数级修正；3）引入熵感知动态加权机制，抑制高熵、不可靠的蒸馏目标，强调置信度高的目标。

Result: 在5个基准测试和2个模型规模上评估：SRPO同时实现了SDPO的快速早期改进和GRPO的长时稳定性，持续超越两个基线的峰值性能。在Qwen3-8B上将5个基准测试平均性能提升3.4%（相比GRPO）和6.3%（相比SDPO），同时产生适中的响应长度，并将每步计算成本降低高达17.2%。

Conclusion: SRPO通过统一框架有效解决了现有RLVR方法的局限性，结合了GRPO和SDPO的优势，在保持计算效率的同时实现了更好的性能和训练稳定性，为大语言模型的后训练提供了更有效的策略优化方法。
...
TL;DR: 提出HCCS（Head-Calibrated Clipped-Linear Softmax）作为softmax的替代方案，针对AMD Versal AI Engines优化，使用int8整数运算提升小模型低精度推理时的计算效率。


<details>
  <summary>Details</summary>
Motivation: Transformer模型中Multi-Head Attention的softmax函数在低精度推理（特别是小模型）时成为计算瓶颈，指数运算和归一化操作带来显著开销。现有AMD平台实现依赖bfloat16或查找表，无法充分利用AI Engine的高吞吐整数向量处理单元。

Method: 提出HCCS：使用基于最大中心化注意力logits的裁剪线性映射作为指数softmax的有界单调替代函数。引入轻量级校准参数，基于代表性数据集离线优化，为每个注意力头单独校准以保持统计特性。针对AMD AI Engines的int8乘累加单元进行硬件优化实现。

Result: HCCS在AMD AI Engines上显著超越其他参考实现的速度性能，同时在小模型或重度量化MHA工作负载上经过量化感知重训练后保持竞争性的任务准确率。这是首个针对AMD AI Engines的int8优化softmax替代方案。

Conclusion: HCCS有效解决了softmax在低精度推理中的计算瓶颈问题，通过硬件友好的int8整数运算实现高性能，同时通过头级校准保持模型质量，为Transformer模型在边缘设备上的高效部署提供了实用解决方案。
...
TL;DR: 提出go-mHC方法，通过广义正交随机矩阵理论实现对双随机矩阵集合的精确参数化，计算复杂度为O(d³)，在保持高效的同时获得完全表达能力


<details>
  <summary>Details</summary>
Motivation: 双随机矩阵能够实现残差流之间的学习混合，但现有方法要么计算复杂度呈阶乘增长（精确方法），要么表达能力有限（Kronecker分解方法），需要一种既高效又具有完全表达能力的精确参数化方法

Method: 基于广义正交随机矩阵理论提出新的精确参数化方法，引入超参数s在计算效率和完全表达能力之间连续插值；将这种方法实例化为go-mHC，并自然地与Kronecker分解方法结合

Result: go-mHC比Kronecker分解基线更完全地填充Birkhoff多面体；在合成流混合任务中达到理论最小损失，收敛速度提升达10倍；在3000万参数GPT风格语言模型中验证有效

Conclusion: go-mHC在表达能力、效率和精确性方面提供了实用途径，可将d作为模型容量的新维度进行扩展
...
TL;DR: BCR是一种通过批量上下文强化训练的单阶段方法，让LLM在共享上下文中同时解决多个问题，实现推理效率提升而不牺牲准确性。
...
Method: 提出批量上下文强化（BCR）方法：训练模型在共享上下文窗口中同时解决N个问题，仅通过每个实例的准确性进行奖励。这种结构修改创建了隐式token预算，无需显式长度监督。

Result: 1) 发现任务缩放定律：推理时并发问题数N增加，每个问题的token使用单调减少，准确性下降远低于基线；2) BCR挑战传统准确率-效率权衡，在标准单问题推理中实现"免费午餐"现象，token使用减少15.8%-62.6%同时保持或提高准确性；3) 模型自主消除冗余元认知循环；4) 隐式预算约束避免了显式长度惩罚的对抗梯度和灾难性优化崩溃。

Conclusion: BCR证明简单的结构激励可以解锁LLM中的潜在高密度推理能力，提供了一种高度稳定的、基于约束的长度控制替代方案，为高效推理提供了实用且有效的方法。
...
### [60] [CIVIC: Cooperative Immersion Via Intelligent Credit-sharing in DRL-Powered Metaverse](https://arxiv.org/abs/2604.02284)
*Amr Aboeleneen,Mohamed Abdallah,Aiman Erbad,Amr Salem*

Main category: cs.NI

TL;DR: CIVIC框架通过智能信用共享机制优化多个Metaverse服务提供商之间的资源分配，提升用户沉浸体验，在动态负载下显著提高请求完成率和客户服务数量。


<details>
  <summary>Details</summary>
Motivation: Metaverse面临复杂的资源分配挑战，包括多样化的虚拟环境、数字孪生、动态用户需求和严格的沉浸体验要求。现有方法无法有效协调多个服务提供商之间的资源共享。

Method: 提出CIVIC框架，整合虚拟环境渲染、数字孪生同步、信用共享和沉浸感知资源供应。将资源分配问题建模为两个NP-hard挑战：非合作设置和基于通用信用池的合作设置。使用深度强化学习调整资源和管理合作服务提供商。

Result: CIVIC实现了12-36%更高的请求完成率，23-70%更高的履行率，20-60%更多的服务客户，以及高达51%更公平的请求分配，同时保持有竞争力的成本。在动态负载和意外需求激增下表现出良好的适应性和鲁棒性。

Conclusion: CIVIC框架通过智能信用共享机制有效解决了Metaverse中多服务提供商的资源分配问题，显著提升了用户沉浸体验和系统性能，适用于现实世界的分布式Metaverse基础设施。
